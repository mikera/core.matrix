(ns clojure.core.matrix.impl.default
  ...
  #?@(:clj [[clojure.core.matrix.impl.double-array :as da]
            [clojure.core.matrix.macros :refer [TODO error scalar-coerce c-for doseq-indexed java-array? array?]]
            [clojure.core.matrix.macros-clj [try-current-implementation *trying-current-implementation* eps==]]]))
  #?(:clj (:import [clojure.lang ISeq])
     :cljs (:require-macros 
             [clojure.core.matrix.macros :refer [TODO error scalar-coerce c-for doseq-indexed java-array? array?]]
             [clojure.core.matrix.macros-cljs :refer [try-current-implementation *trying-current-implementation* eps==]]
             )))
...
(extend-protocol mp/PDimensionInfo
  nil
    ...
    (is-scalar? [m] true)
    (is-vector? [m] false)
    (get-shape [m] (js/console.log "shape of a number...") nil)
  ...
  #?(:clj Object :cljs object)
    ...
    (get-shape [m]
      (js/console.log "shape of an object...")
      (cond
        #?(:clj (.isArray (.getClass m)) :cljs (= js/Array (type m)))
          (let [n (count m)]
